...
{ 	
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //IRQ 
	GPIO_Init(GPIOA, &GPIO_InitStructure);
  ...
  SPI2_Init();    		//初始化SPI	 
	...
	delay_ms(100);
 	...
 	while(NRF24L01_Check()){	//检查NRF24L01是否在位.	
		...
		#endif
		delay_ms(200);
		SYS_LED=!SYS_LED;    //nrf24l01初始化失败，指示灯快闪
	}				
...
// 	NRF24L01_Write_Reg(WRITE_REG_NRF+RF_SETUP,0xf);  //0x27;250k,设置TX发射参数,0db增益,250kbps,低噪声增益开启   
// 	NRF24L01_Write_Buf(WRITE_REG_NRF+TX_ADDR,(u8*)TX_ADDRESS,TX_ADR_WIDTH);//写TX节点地址 
// 	NRF24L01_Write_Reg(WRITE_REG_NRF+RF_CH,Config.WxFQ);       	
// 	NRF24L01_CE=1;	  

	
		NRF24L01_RX_Mode(Config.WxFQ);
}
//检测24L01是否存在
//返回值:0，成功;1，失败	
...
//	NRF24L01_Write_Reg(WRITE_REG_NRF+EN_AA,0x01);     //使能通道0的自动应答    
	...
	NRF24L01_Write_Reg(WRITE_REG_NRF+RX_PW_P0,RX_PLOAD_WIDTH);//选择通道0的有效数据宽度 	    
	NRF24L01_Write_Reg(WRITE_REG_NRF+RF_SETUP,0xf);  //0x27;250k 设置TX发射参数,0db增益,250kbps,低噪声增益开启   
	NRF24L01_Write_Reg(WRITE_REG_NRF+CONFIG,0x0e);    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,发送模式,开启所有中断
