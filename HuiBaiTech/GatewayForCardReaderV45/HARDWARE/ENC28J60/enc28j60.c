...
static void ENC28J60_SPI1_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOB, ENABLE);//PORTB,D,G时钟使能 
 
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;				 			//ENC28J60_INT
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 		//
	GPIO_Init(GPIOB, &GPIO_InitStructure);					 			//
	GPIO_SetBits(GPIOB,GPIO_Pin_1);						 						//
			

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3|GPIO_Pin_4;	//ENC28J60_RST  ENC28J60_CS		 
...
void ENC28J60_PHY_Write(u8 addr,u32 data)
{
	...
	ENC28J60_Write(MIWRL,(u8)(data& 0x00ff));		//写入数据
	ENC28J60_Write(MIWRH,(u8)(data>>8));		   
...
{		
	...
	if(retry>=500)//ENC28J60初始化失败
	{
		printf("ENC28J60初始化失败\r\n");
		return 1;
	}
	// do bank 0 stuff
	...
	ENC28J60_Write_Op(ENC28J60_BIT_FIELD_SET,MACON3,MACON3_PADCFG0|MACON3_TXCRCEN|MACON3_FRMLNEN|MACON3_FULDPX);//different momo
	...
	tmpbf[0] = ENC28J60_Read(MAADR5);
	tmpbf[1] = ENC28J60_Read(MAADR4);
	tmpbf[2] = ENC28J60_Read(MAADR3);
	tmpbf[3] = ENC28J60_Read(MAADR2);
	tmpbf[4] = ENC28J60_Read(MAADR1);
	tmpbf[5] = ENC28J60_Read(MAADR0);
	
//	if(ENC28J60_Read(MAADR5)== macaddr[0])
	if(memcmp(tmpbf,macaddr,6) == 0)//mac地址相等
	{
		printf("mac相等，地址读出来为：%d:%d:%d:%d:%d:%d\r\n",tmpbf[0],tmpbf[1],tmpbf[2],tmpbf[3],tmpbf[4],tmpbf[5]);
		return 0;//初始化成功
	}
	else 
	{
		printf("mac地址不相等读出来为：%d:%d:%d:%d:%d:%d\r\n",tmpbf[0],tmpbf[1],tmpbf[2],tmpbf[3],tmpbf[4],tmpbf[5]);
		return 1; 
	}	  
